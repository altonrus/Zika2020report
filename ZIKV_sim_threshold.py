...
class Sim_Model:
    ...
    def zeroOutcomes(self):
        self.TP = [0] * 3  # true positives accrued
        self.FP = [0] * 3  # false positives accrued
        self.costBC = [0] * 3  # cost accrued related to testing/TP/FP
    ...
    #@profile
    def testForZika(self, prev):
        # i = scenario; j=donor group

        # TPmult and FPmult(iplier) are the TP and FP calculation excluding the 'number
        #  of units' component. Done separately to enable accurate cost of FP/TP 
        #  calculations. PropTransfused = Proportion transfused.

        #Create vector with the test characteristics (sens, spec, )

        # Extract test characteristics based on scenario indicated
        # for donor group and policy

        #print(testChar, "prev", prev)
        #testChar[i,0] = cost; testChar[i,1] = sens; testChar[i,2] = spec
        propTransfused = np.empty([3])
        TPmult = np.empty([3])
        FPmult = np.empty([3])
        propTransfused = (1 - self.tests[:,1]) * prev + self.tests[:,2] * (1 - prev)
        # print("prev", prev, "Sens", self.tests[:,1], "Spec",  self.tests[:,2])
        TPmult = prev * self.tests[:,1] / propTransfused[:]
        FPmult = (1 - prev) * (1 - self.tests[:,2]) / propTransfused[:]
        # True positives, false positives, test-related costs calculations

        donorUnits = (self.unitsRBC + self.unitsPLT+self.unitsFFP)/ 1.4210
        self.TP = TPmult*donorUnits
        self.FP = FPmult*donorUnits
        
        self.costBC = (donorUnits  # (per donor unit test cost)*(est donor units for recipient + TP + FV)
                                   * (self.tests[:,0]
                                   + (self.costTP+self.tests[:,0]) * (TPmult+FPmult)))
        self.costBC = (donorUnits  # (per donor unit test cost)*(est donor units for recipient + TP + FV)
                                   * (self.tests[:,0]
                                   + (self.costTP+self.tests[:,0]) * (TPmult+FPmult)))
        # print("TPmult", TPmult, "FPmult", FPmult, "propTransfused", propTransfused, "costBC", self.costBC)

        # Zika positive units calculations.
        probNegUnit = np.empty([3])
        probNegUnit = 1-(prev * (1 - self.tests[:,1])/propTransfused)
    ...
    def getColHeaders(self, PSA):
        ...
        cols = cols + ["costBC"+str(i) for i in range(3)]
        cols = cols + ["TP" + str(i) for i in range(3)]
        cols = cols + ["FP" + str(i) for i in range(3)]
        cols = cols + ["probTTZ" + str(i) for i in range(3)]
        cols = cols + ["costRecip" + str(i) for i in range(3)]
        cols = cols + ["costInfant" + str(i) for i in range(3)]
        cols = cols + ["costPartner" + str(i) for i in range(3)]
        cols = cols + ["costPartnerInfant" + str(i) for i in range(3)]
        cols = cols + ["costProductivity" + str(i) for i in range(3)]
        cols = cols + ["QALYLrecip" + str(i) for i in range(3)]
        cols = cols + ["QALYLinfant" + str(i) for i in range(3)]
        cols = cols + ["QALYLpartner" + str(i) for i in range(3)]
        cols = cols + ["QALYLpartnerInfant" + str(i) for i in range(3)]
        cols = cols + ["CZS" + str(i) for i in range(3)]
        cols = cols + ["GBS" + str(i) for i in range(3)]
        cols = cols + ["flulike" + str(i) for i in range(3)]
        return cols
    ...
    def get_PSA_row(self):
        row = np.array([self.p_flu_like_symptoms,
                            ...
                            self.separateInventoryPerUnitCost,
                            ...
                            self.tests[1][0], #cost ID-NAT
                            self.tests[1][1], #Sens ID-NAT
                            self.tests[1][2], #spec ID-NAT
                            self.tests[2][0], #Cost MP-NAT
                            self.tests[2][1], #Sens MP-NAT
                            self.tests[2][2], #Spec MP-NAT
    ...
    #@profile
    def doRep(self, prev, PSA):# Runs simulation for one recipient
        ...
        # Perform blood center analysis for each policy
        # FP, TP, testing cost, ZIKV+ RBC/PLT/FFP units,
        # Transfusion-transmission of ZIKV
        self.testForZika(prev)
        #print("PPreg", p_pregnant, "Prob TTZ", self.probTTZ)
        self.outcomes(0)
        if p_pregnant == 0:
            row = sim.getRow(prev, PSA)
        else:
            row = sim.getRow(prev, PSA)*(1-p_pregnant)
            self.outcomes(1)
            #print("PPreg", p_pregnant, "Prob TTZ", self.probTTZ[0], "CZS", self.CZS)
            row = row + sim.getRow(prev, PSA)*p_pregnant

        return row

    #@profile
    def runIter(self, prev, PSA):
    ...
    def runModel(self, n, prev, PSA):
        ...
        output = self.runIter(prev, PSA)
        output['Prevalence'] = prev
        if n==1:
            result = output
        else:
            output['Iter'] = 0
...
if __name__ == "__main__":
    ...
    output, timeString = sim.runModel(n, prev, PSA)
